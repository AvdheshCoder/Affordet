...
public class PaymentController {
	...
	public ModelAndView authorizePayment(HttpServletRequest httpReq,
		...
		try {
			...
			URL url = new URL("https://api.razorpay.com/v1/payments/"
					+ paymentId + "/capture");
			String encoding = Base64Encoder
					.encode("rzp_test_fELiPnnFZxkgi2:RlJ7TtXaChIayI32Q5U4eRs4");
			...
			try {
				com.avd.common.util.SendMail
						.sendEmailimg(email, "Mohini Order Id:" + orderId
								+ "Transaction Successful", mailMessg,
								inlineImages);
			} catch (Exception e) {
			...
			return new ModelAndView(
					"redirect:paymentSuccessful?dsvGfTRFekjhgdfnm32Gjfh="
							+ orderId + "&amount=djsT565yhFG");
		}
	...
	public ModelAndView authorizeOrder(HttpServletRequest httpReq,
		...
		try {
			...
			String mailMessg1 = "<h3>An order is registered with you</h3>"
					+ "Order is registered"
					+ "<br />ORDER ID:"
					+ orderId
					+ " <br />Please see details of order and accept order from your dashboard."
					+ "<br />Keep Smiling <br /><br /><br /><table border=\"1\"><tr><td> </td><td> </td></table>"
					+ "Best Regards,<br />Balaji MDM Team<br />";
			
			try {
				com.avd.common.util.SendMail.generateAndSendEmail(email,
						"Balaji Traders, Order Details ", mailMessg);
				com.avd.common.util.SendMail.generateAndSendEmail("mdmbalaji@gmail.com",
						"Balaji Traders, Order Request ", mailMessg1);
			} catch (Exception e) {
				System.out.println("not sent");
			}
