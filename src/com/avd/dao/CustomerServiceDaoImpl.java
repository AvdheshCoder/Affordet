...
public class CustomerServiceDaoImpl extends HibernateDaoSupport implements CustomerServiceDao {
	...
	public List<Object[]> getProductInfo(Map<String, Object> map) {
		...
		try {
			SQLQuery qry = session.createSQLQuery(
					"select t1.product_id, t1.product_name,t1.discount, t2.category_id,t2.sub_category_id,t1.discounted_price , t1.actual_price, "
	...
	public List<Object[]> getSelectedProductImages(Map<String, Object> map) {
		...
		Session session = sessionFactory.openSession();
		try {
			SQLQuery qry = session.createSQLQuery("select t1.p_id, t1.product_image_large,t1.product_image_medium  "
					+ "  from tbl_product_images t1" + " where t1.category_id='" + map.get("categoryId").toString()
					+ "' and t1.sub_category_id='" + map.get("subCategoryId").toString() + "'and t1.p_id='"
					+ map.get("productId").toString() + "' order by t1.image_id");
			productInfo = qry.list();
	...
	public String signup(Map<String, Object> map) {
		...
		try {
			...
			SQLQuery qry = session.createSQLQuery("select max(login_id)+1 ,max(login_id) as a from user_login");
			productInfo = qry.list();
		...
		try {
			...
			for (Object[] obj : productInfo) {
				...
				qry = session.createSQLQuery(
						"select category_Id, sub_category_Id,sub_category_desc from lu_sub_category where category_id='"
								+ product.getCategoryId() + "' and is_active=1");
				productInf = qry.list();
	...
	public String checkEmail(Map<String, Object> map) {
		...
		try {
			...
			SQLQuery qry = session.createSQLQuery(
					"select email_Id ,user_name from user_login where email_id='" + map.get("email").toString() + "'");
			productInfo = qry.list();
			for (Object[] reslt : productInfo) {
	...
	public String authenticateSignup(Map<String, Object> map) {
		...
		String flag = "0";
		try {
			SQLQuery qry = session.createSQLQuery("select valid_code ,login_Id  from user_login where valid_code='"
					+ map.get("validCode").toString() + "' and is_active='0' and flag='0'");
			productInfo = qry.list();
			for (Object[] reslt : productInfo) {
	...
	public String getCartProducts(Map<String, Object> map) {
		...
		try {
			...
			SQLQuery qry = session
					.createSQLQuery("select count(product_id) ,count(user_id) from tbl_cart_record where user_id='"
	...
	public String getCartProductId(Map<String, Object> map) {
		...
		String flag = "0";
		try {
			SQLQuery qry = session.createSQLQuery("select product_id , quantity  from tbl_cart_record where user_id='"
					+ map.get("loginId").toString() + "'");
			productInfo = qry.list();
			String id = "";
	...
	public Integer getArtificalId() {
		...
		try {
			...
			ps = conn.prepareStatement("select max(id)  as cnt from USER_VISIT_LOG");
			rs = ps.executeQuery();
			...
			ps = conn.prepareStatement("INSERT INTO  USER_VISIT_LOG (ID) VALUES" + "  (" + productId + ")");
	...
	public List<Object[]> getCartProductsDesc(Map<String, Object> map) {
		...
		try {
			SQLQuery qry = session.createSQLQuery(
					"select t1.product_id, t1.product_name,t1.discount, t2.category_id,t2.sub_category_id,t1.discounted_price , t1.actual_price,t3.quantity ,t1.discounted_price * t3.quantity "
	...
	public List<Object[]> getReceiverAddress(Map<String, Object> map) {
		...
		try {
			...
			if ("3".equals(map.get("flag").toString())) {
				SQLQuery qry = session.createSQLQuery(
						"select email_id , name, order_id , total_amount , address , mobile_no , pin_code "
								+ "  from product_final_orders_information " + " where  login_id='"
								+ map.get("loginId").toString() + "' and order_id='" + map.get("orderId").toString()
								+ "'");
				productInfo = qry.list();
			} else {
				SQLQuery qry = session.createSQLQuery(
						"select email_id , name, order_id , total_amount , address , mobile_no , pin_code "
								+ "  from product_final_orders_information " + " where  login_id='"
								+ map.get("loginId").toString() + "' order by order_date desc  limit 1");
				productInfo = qry.list();
			}
	...
	/*
	 * select t1.quantity*t2.discounted_price from tbl_cart_record t1 inner join
	 * tbl_product_information t2 on t1.product_id=t2.product_id
	 */
	...
	public List<Object[]> updateFinalCartProducts(Map<String, Object> map) {
		...
		try {
			conn = new JdbcConnection().getConnection();
			ps = conn.prepareStatement("update product_final_orders set quantity='" + map.get("quantity").toString()
					+ "' where order_id='" + map.get("orderId").toString() + "' and product_id='"
	...
	public List<Object[]> getOrderDetails(Map<String, Object> map) {
		...
		try {
			...
			SQLQuery qry = session.createSQLQuery("select t4.*, t2.status from product_final_orders_information t4 "
					+ " inner join lu_order_status t2 on t2.id=t4.status_id " + " where t4.customer_finalize=1 and "
					+ toAppend);
			productInfo = qry.list();
		...
		try {
			...
			if ("1".equals(map.get("flag").toString())) {
				ps = conn.prepareStatement(
						"update product_final_orders_information set total_Amount='" + map.get("totalAmount").toString()
								+ "' where order_id='" + map.get("orderId").toString() + "'");
				ps.executeUpdate();
			} else if ("2".equals(map.get("flag").toString())) {
				String deliveryTime = "7";
				SQLQuery qry = session.createSQLQuery(
						"select max(delivery_time ),'1' from tbl_product_information t1 where t1.product_id in (select product_id from product_final_orders where order_id='"
				...
				ps = conn.prepareStatement(
						"delete from  tbl_cart_record  where user_id='" + map.get("loginId").toString()
								+ "' and product_id in (select product_id from product_final_orders where order_id='"
								+ map.get("orderId").toString() + "' )");
		...
		try {
			SQLQuery qry = session.createSQLQuery(
					"select t1.product_id, t1.product_name,t1.discount, t2.category_id,t2.sub_category_id,t1.discounted_price , t1.actual_price, "
							+ " t2.product_images from tbl_product_information t1"
							+ " inner join tbl_product_display_images t2 on t2.p_id= t1.product_id and t2.category_id=t1.category_id and t1.sub_category_id=t2.sub_category_id"
							+ " where t1.show_id='" + map.get("showId").toString()
							+ "' and t1.is_active=1 and t1.quantity>0");
			productInfo = qry.list();
	...
	public List<Object[]> productOfTheDay(Map<String, Object> map) {
		...
		try {
			...
			if ("1".equals(map.get("slideFlag").toString())) {
				...
				qry = session.createSQLQuery(
						"select t1.id, t1.image " + "  from tbl_slide_show t1" + " where t1.is_active=1 ");
				productInfo = qry.list();
	...
	public List<Object[]> getProductReview(Map<String, Object> map) {
		...
		Session session = sessionFactory.openSession();
		try {
			SQLQuery qry = session.createSQLQuery("select t1.product_id, t1.review from product_reviews t1"
	...
	public List<Object[]> getTrackDesc(Map<String, Object> map) {
		...
		try {
			SQLQuery qry = session.createSQLQuery(
					"select t1.product_id, t1.product_name,t1.discount, t2.category_id,t2.sub_category_id,t1.discounted_price , t1.actual_price,t3.quantity,t3.amount * t3.quantity "
	...
	public String getNextStatus(String orderId) {
		Session session = sessionFactory.openSession();
		List<Object[]> statusData = new ArrayList<Object[]>();
		String result = null;
		try {
			SQLQuery qry = session.createSQLQuery("select min(id), status from lu_order_status where id > "
					+ "(select status_id from product_final_orders_information where order_id = '" + orderId + "')");
			statusData = qry.list();
			if (statusData != null && statusData.size() > 0)
				result = statusData.get(0)[0].toString() + "," + statusData.get(0)[1].toString();
		} catch (Exception ex) {
			ex.printStackTrace();
		} finally {
			if (session != null) {
