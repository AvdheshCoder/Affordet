...
.form-left input[type="button"] {
	background: #282828 url("../images/submit-btn1.png") no-repeat scroll 0
		0;
...
.form-control:focus {
	...
	-webkit-box-shadow: inset 0 1px 1px rgba(0, 0, 0, .075), 0 0 8px
		rgba(102, 175, 233, 0.83);
	box-shadow: inset 0 1px 1px rgba(0, 0, 0, .075), 0 0 8px
		rgba(102, 175, 233, 0.75);
}
...
.lb-close-btn {
	background: rgba(0, 0, 0, 0) url("images/into.png") no-repeat scroll
		-5px -2px;
	cursor: pointer;
	height: 27px;
	margin: 4px 0px 0 0;
	...
	<div class="alpha30">
						...
						<tr>
							...
							<td width="50%"><h4>
									Status <font color="green"> <c:set var="stat"
											value="${listItem[26]}"></c:set> <u>${listItem[26]} </u></font>
								</h4></td>
						</tr>
						<tr>
							<td width="50%">Name: ${listItem[18]}</td>
							<td width="50%">Email ${listItem[0]}</td>
						</tr>
						<tr>
							<td width="50%">Mobile No ${listItem[4]}</td>
							<td width="50%">Address ${listItem[3]}</td>
						</tr>
						<tr>
							<td>Order Date: <fmt:formatDate pattern="dd-MM-yyyy"
									value="${listItem[6]}" />
							</td>
							<td>Delivery Date: <c:set var="tot" value="${listItem[2]}"></c:set>

								<c:set var="sellFlag" value="${listItem[16]}"></c:set> <c:choose>
									<c:when test="${sellFlag eq '1'}">
										<fmt:formatDate pattern="dd-MM-yyyy" value="${listItem[8]}" />
									</c:when>
									<c:otherwise>
										<fmt:formatDate pattern="dd-MM-yyyy" value="${listItem[7]}" />
									</c:otherwise>
								</c:choose>
							</td>
						</tr>
	 ...
	 -->
				<form name="sellerform" id="sellerform" method="post">
					<div class="containe"
						style="box-shadow: 10px 10px 5px #888888; margin:">
							...
							<tbody style="padding: 1px;">
								<c:forEach var="listItem" items="${map.cartProductsDesc1}"
									...
									<tr style="border: 1px solid; border-color: threedlightshadow;">
										<td colspan='2'><img
											src="data:image/png;base64, ${listItem[9]}" height="44"
											width="44" alt="" /> &nbsp;&nbsp;&nbsp;${listItem[1]}</td>

										<td width="20%" align="center">${listItem[7]}</td>

										<td width="20%" align="center">${listItem[11]}</td>
